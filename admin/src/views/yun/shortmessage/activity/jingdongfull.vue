    ...
    <d2-container>
        <div class="messagePromotionAdd mytable">
            ...
            <div class="top_div">
                ...
                <div class="pub_div">原价：</div>
                <el-input  class="pub_div" v-model="nowData.percentage" placeholder="请输入内容" style="width:120px;"></el-input>
                <el-button  class="pub_div" type="primary" @click="saveTop" :loading="SaveStatus" style="float: right;">保存</el-button>
            </div>
            <!-- 数据列表 -->
            <div class="data_list">
                <div class="data_list_item" v-for="(item, index) in dataList" :key="index">
                    <div class="div_pub">优惠金额：<el-input v-model="item.percentage" placeholder="请输入内容" @blur="blur(item.id)"></el-input></div>
                    <div class="div_pub">订购数：<el-input v-model="item.num" placeholder="请输入内容" @blur="blur(item.id)"></el-input></div>
                </div>
            </div>
        </div>
    </d2-container>
...
export default {
    ...
    methods: {
        ...
        goWebView(){
            const newUrl = window.location.protocol + '//' + document.domain + "/jingdongfull/index.html"
            console.log(newUrl)
        ...
        getServiceData: function () {
            ...
            var query = '?page=1&page_size=50&type=3&type_a=103'
